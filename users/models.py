...
class PointLog(models.Model):
    ...
    def __str__(self):
        return f"{self.user_profile.user.username} - {self.points_awarded} points for {self.reason or 'Unspecified'}"
...
def check_and_award_badges(user_instance):
    ...
    # Example Badge: "First Steps" - Awarded after completing 1 lesson
    try:
        ...
        if not UserBadge.objects.filter(user_profile=profile, badge=first_steps_badge).exists():
            from courses.models import UserLessonProgress # Local import to avoid circularity if moved
            if UserLessonProgress.objects.filter(user=user_instance).count() >= 1:
                UserBadge.objects.create(user_profile=profile, badge=first_steps_badge)
                # Optionally send a message/notification
    except Badge.DoesNotExist:
        pass # Badge not defined, or handle error

    # Example Badge: "Point Collector" - Awarded after reaching 100 points
    try:
        point_collector_badge = Badge.objects.get(slug='point-collector') # Assumes this badge exists
    ...
    # Example Badge: "Forum Contributor" - Awarded after 5 forum posts
    try:
        ...
        if not UserBadge.objects.filter(user_profile=profile, badge=forum_contributor_badge).exists():
            from forum.models import ForumPost # Local import
